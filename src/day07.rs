...
fn get_hand_strength(cards: &str, is_part2: bool) -> (usize, usize) {
    ...
    let counts = card_by_count.iter().filter(|&(&k, _)| k != 'J' || !is_part2).map(|(_, &v)| v).collect_vec();
    let jack_count = if is_part2 {*card_by_count.get(&'J').unwrap_or(&0)} else {0};
    ...
    let idx = cards.chars().fold(0, |acc, c| {
        (acc << 4) + card_strength(c, is_part2)
    });
    println!("idx: {}", idx);
...
pub fn solution() -> (usize, usize) {
    ...
    let mut cards = REAL_INPUT.lines().map(|l| {
        let (hand, bid) = l.split_once(" ").unwrap();
        (bid.parse().unwrap(), get_hand_strength(hand, false), get_hand_strength(hand, true))
    }).collect_vec();
    cards.sort_unstable_by_key(|&(_, k, _)| k);
    let part1 = cards.iter().enumerate().map(|(i, (bid, _, _))| {
        bid * (i + 1)
    }).sum::<usize>();
    cards.sort_unstable_by_key(|&(_, _, k)| k);
    let part2 = cards.iter().enumerate().map(|(i, (bid, _, _))| {
        bid * (i + 1)
    }).sum::<usize>();
